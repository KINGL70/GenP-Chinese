...
Else
	...
	If $sCountProblem <> "" Then
		MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "GenP - 配置不完整", _
				"config.ini 版本匹配 (" & $g_Version & ")，但各节未包含" & @CRLF & _
				"此版本预期的条目数量：" & @CRLF & @CRLF & _
				$sCountProblem & @CRLF & _
				"配置可能被截断或手动编辑过。" & @CRLF & _
				"修补、防火墙和 hosts 结果可能不完整。" & @CRLF & _
				"请替换为 " & $g_Version & " 版本附带的 config.ini。")
	EndIf
EndIf
...
Func _CheckPatchStatesVersion()
	...
	If $iPrior < $iCurrent Then
		Local $iResult = MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING, $MB_SYSTEMMODAL), _
				"需要升级版本", _
				"您的 patch_states.ini 不兼容" & @CRLF & _
				"当前 GenP " & $g_Version & " / 配置 " & $ConfigVerVar & @CRLF & _
				"为正常运行，您必须：" & @CRLF & _
				"1. 删除 patch_states.ini" & @CRLF & _
				"2. 重新运行 GenP 以创建新基线" & @CRLF & @CRLF & _
				"不执行此步骤可能导致黑屏。" & @CRLF & @CRLF & _
		...
		If $iResult = $IDYES Then
			...
			EndIf
		Else
			MsgBox($MB_ICONWARNING, "自行承担风险", _
					"如果遇到问题，请删除 patch_states.ini 并重新运行。")
		EndIf
	EndIf
EndFunc
...
While 1
	...
	Select
		...
		Case $idMsg = $idButtonSearch
			$fInterrupt = 0
			$g_bIsPatching = True
			GUICtrlSetData($idLog, "活动日志" & @CRLF & "- - - - - - - - - - -" & @CRLF & @CRLF & "GenP Version: " & $g_Version & @CRLF & "Config Version: " & $ConfigVerVar & @CRLF)
		...
		Case $idMsg = $idBtnCure
			...
			If $sCureTgtWhy <> "" Then
				MsgBox(BitOR($MB_OK, $MB_ICONERROR), "无法修补 - 目标文件夹", $sCureTgtWhy)
			...
			If $sCureCfgWhy <> "" Then
				Local $iCureGo = MsgBox(BitOR($MB_YESNO, $MB_DEFBUTTON2, $MB_ICONWARNING, $MB_SYSTEMMODAL), _
						...
						"现在修补可能会应用不完整或错误的模式集，可能导致" & @CRLF & _
						"Adobe 应用无法启动（黑屏）。仍要继续修补吗？")
				If $iCureGo <> $IDYES Then
					MemoWrite(@CRLF & "修补已取消：config.ini 验证失败。" & @CRLF)
			...
			If $bAppsSelected And $bEnableNGLFirewall = 1 Then
				If _EnableNGLFirewallRules(False) = -99 Then
					MemoWrite(@CRLF & "修补已取消 - 请添加您的第三方防火墙规则后再修补。")
					ContinueLoop
				EndIf
			EndIf
			...
			If $g_bInModifiedMode Then
				...
				If $iStillTodo = 0 Then
					MemoWrite(@CRLF & "修改工作队列已清空。")
					LogWrite(1, "Modified work queue drained - all files patched.")

					If $bEnableNGLFirewall = 1 Then
						MemoWrite(@CRLF & "注意：NGL 防火墙隔离规则已激活；网络通道已锁定。")
						MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "网络隔离已激活", _
								"NGL 防火墙规则正在成功监控活动模块。" & @CRLF & @CRLF & _
								"防火墙规则不强制要求完全重启系统，" & @CRLF & _
								"但建议关闭并重新打开应用程序以应用状态。")
					EndIf
					...
					MemoWrite(@CRLF & "修补完成。" & @CRLF & "您可以继续使用。")
				Else
					MemoWrite(@CRLF & "未修补：" & $iStillTodo & " 个文件仍需修补（再次点击修补）。")
			...
			If $bFoundAcro32 = True Then
				MsgBox($MB_SYSTEMMODAL, "信息", "GenP 不修补 32 位版本的 Acrobat。" & @CRLF & @CRLF & "请使用 64 位版本的 Acrobat。")
			...
			If $bFoundGenericARM = True Then
				MsgBox($MB_SYSTEMMODAL, "信息", "此 GenP 版本不支持 ARM 二进制文件，仅支持 x64。")
		...
		Case $idMsg = $idBtnRestore
			...
			If $sRestTgtWhy <> "" Then
				MsgBox(BitOR($MB_OK, $MB_ICONERROR), "无法还原 - 目标文件夹", $sRestTgtWhy)
			...
			If UBound($aUnrestoredInUse) > 0 Then
				Local $sUnrestMsg = UBound($aUnrestoredInUse) & " 个文件因仍在使用中而无法还原：" & @CRLF & @CRLF
				For $iU = 0 To UBound($aUnrestoredInUse) - 1
					$sUnrestMsg &= "  - " & $aUnrestoredInUse[$iU] & @CRLF
				Next
				$sUnrestMsg &= @CRLF & "其备份 (.bak) 已保留。关闭所有打开的" & @CRLF & "Adobe 应用并再次运行还原以完成。"
		...
		Case $idMsg = $idClearLicCaches
			If Not _IsChecked($idClearLicCaches) Then
				Local $iKeepLic = MsgBox(BitOR($MB_YESNO, $MB_ICONQUESTION, $MB_DEFBUTTON1, $MB_SYSTEMMODAL), _
						"清除许可证缓存  -  推荐", _
						"修补后清除许可证缓存是推荐的，默认开启。" & @CRLF & @CRLF & _
						"保持开启会清除过时的许可证状态，避免应用在修补后" & @CRLF & _
		...
		Case $idMsg = $idUseCustomDefault
			If _IsChecked($idUseCustomDefault) Then
				Local $sPickedPath = _BrowseForFolderDialog("选择启动时打开的文件夹", $MyhGUI)
				If $sPickedPath <> "" Then
		...
		Case $idMsg = $idBtnSaveOptions
			...
			If $bLaunchOlderVerDl Then
				...
				Else
					...
					MemoWrite("下载将保存到：" & $sOvdDest)
					Local $iPID = Run('cmd.exe /k "title Adobe Older Versions Downloader & color 0F & mode con cols=100 & .\main.exe"', $g_sOVD_DIR, @SW_SHOW)
					If $iPID = 0 Then
		...
		Case $idMsg = $idBtnDummyAGS
			...
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION, $MB_SYSTEMMODAL), _
					"需要重启", _
					"虚拟 AGS 组件已安装。" & @CRLF & @CRLF & _
					"需要重启设备才能使更改生效。" & @CRLF & _
					"GenP 将立即重启您的设备。请保存所有打开的工作" & @CRLF & _
					"在其他应用中，然后继续。")
		...
		Case $idMsg = $idBtnSetTrustPath
			Local $sSelected = _BrowseForFolderDialog("选择 WinTrust 的 Adobe 安装文件夹", $MyhGUI)
		...
		Case $idMsg = $idBtnProxySetup
			...
			If $bOk Then
				...
				MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "安装完成", _
						"mitmproxy 已安装且证书已信任。" & @CRLF & @CRLF & _
						"后续步骤：" & @CRLF & _
						"  - 目标选择（可选）以选择全局或指定应用" & @CRLF & _
						"  - 启动/停止代理来运行并路由 Windows 流量")
			Else
				Local $sErrMsg = "Setup failed."
				Switch @error
					Case 1
						$sErrMsg = "GenP 必须以管理员身份运行。"
					Case 2
						$sErrMsg = "无法部署 mitmdump.exe。" & @CRLF & _
								"请确认 resources\mitmproxy\mitmdump.exe 在构建目录中。"
					Case 3
						$sErrMsg = "无法部署重写脚本。" & @CRLF & _
								"请确认 resources\mitmproxy\mitmproxy_genuine_fullguard.py 在构建目录中。"
					Case 4
						$sErrMsg = "mitmdump 未在 8 秒内生成证书。" & @CRLF & _
								"这通常意味着独立二进制文件无法解压到 %TEMP%。" & @CRLF & _
								"请手动运行一次 mitmdump.exe，然后再次点击安装设置。"
					Case 5
						$sErrMsg = "certutil 安装证书失败（退出码 " & @extended & "）。" & @CRLF & _
								"请确认您以管理员身份运行 GenP。"
				EndSwitch
		...
		Case $idMsg = $idBtnProxyStartStop
			If _IsMitmproxyRunning() Or _IsWindowsProxyOn() Then
				...
				_StopMitmproxy()
				MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "代理已停止", _
					"mitmdump 已停止 / Windows 系统代理已禁用。")
			Else
				Local $bOk = _StartMitmproxy()
				If $bOk Then
					_EnableWindowsProxy()
					Local $sPMode = IniRead($sINIPath, "Options", "ProxyMode", "Global")
					MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "代理已启动", _
						"mitmdump 正在监听 " & $g_sMITM_PROXY & "，Windows 代理已启用。" & @CRLF & @CRLF & _
						"模式：" & (StringLower($sPMode) = "target" ? "指定应用" : "全局（推荐）") & "。")
				Else
					Local $sErrMsg2 = "Start failed."
					Switch @error
						Case 1
							$sErrMsg2 = "mitmproxy 未安装。请先点击安装设置。"
						Case 2, 3
							$sErrMsg2 = "mitmdump 未正常启动。" & @CRLF & _
								"可能原因：独立二进制文件无法解压到 %TEMP%。" & @CRLF & _
								"请在任务管理器中检查旧的 mitmdump 进程。"
						Case 4
							$sErrMsg2 = "此机器上所有代理端口 8080-8089 均被占用。" & @CRLF & _
								"请关闭占用这些端口的程序（其他代理、开发工具）后重试。"
					EndSwitch
		...
		Case $idMsg = $idBtnProxyRemove
			If MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING), "卸载 mitmproxy", _
					...
					"您的 %USERPROFILE%\.mitmproxy\ 文件夹（证书源文件）" & @CRLF & _
					"将被保留以便将来更快地安装。继续？") = $IDYES Then
				...
				Else
					MsgBox(BitOR($MB_OK, $MB_ICONERROR), "卸载失败", _
						"部分卸载失败 (@error=" & @error & ")。请查看 GenP 日志。")
				EndIf
			EndIf
			_RefreshProxyStatus()

		Case $idMsg = $idBtnCreateFW
			MsgBox(64, "防火墙规则", "将扫描 Adobe 应用并应用规则。" & @CRLF & "Windows 防火墙用户将被提示选择应用。" & @CRLF & "第三方防火墙用户请查看日志标签页获取需要添加的路径。")
			ToggleLog(0)
			CreateFirewallRules()
...
Func MainGui()
	...
	$hMainTab = GUICtrlCreateTabItem("主界面")
	...
	_GUICtrlListView_AddColumn($idListview, "  应用文件", 445, 2)
	...
	$idBtnUncheckAll = GUICtrlCreateButton("全部取消", 28, 430, 94, 25)
	...
	$idButtonSearch = GUICtrlCreateButton(" 搜索", 140, 520, 94, 32)
	GUICtrlSetFont(-1, 10, 700)
	...
	$idBtnModified = GUICtrlCreateButton(" 修改项", 363, 520, 94, 32)
	GUICtrlSetFont(-1, 10, 700)
	...
	$g_idHyperlinkMain = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	GUICtrlCreateGroup("首次运行选项", 5, 35, 585, 90)

	$idCreateStates = GUICtrlCreateCheckbox("创建并填充新的 patch_states.ini（使用路径设定的位置）", 15, 60, 460, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	GUICtrlSetState($idCreateStates, $GUI_UNCHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idReconcileStates = GUICtrlCreateCheckbox("核对导入的 patch_states.ini 与当前文件（使用路径设定的位置）", 15, 90, 460, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	GUICtrlSetState($idReconcileStates, $GUI_UNCHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	GUICtrlCreateGroup("", -99, -99, 1, 1)

	GUICtrlCreateGroup("扫描选项", 5, 129, 585, 90)

	$idFindACC = GUICtrlCreateCheckbox("始终搜索 ACC", 15, 154, 278, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	If $bFindACC = 1 Then GUICtrlSetState($idFindACC, $GUI_CHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idEnableMD5 = GUICtrlCreateDummy()

	$idOnlyAFolders = GUICtrlCreateCheckbox("仅在默认命名的文件夹中搜索", 15, 184, 278, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	If $bOnlyAFolders = 1 Then GUICtrlSetState($idOnlyAFolders, $GUI_CHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idUseCustomDefault = GUICtrlCreateCheckbox("使用自定义默认搜索路径：", 300, 154, 278, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	...
	GUICtrlCreateGroup("修补选项", 5, 223, 585, 115)

	$idShowBetaApps = GUICtrlCreateCheckbox("显示 Beta 应用", 15, 248, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	If $bShowBetaApps = 1 Then GUICtrlSetState($idShowBetaApps, $GUI_CHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idEnableGood1 = GUICtrlCreateCheckbox("启用 Good 补丁", 15, 278, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	If $bEnableGood1 = 1 Then GUICtrlSetState($idEnableGood1, $GUI_CHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idShowLaunchBar = GUICtrlCreateCheckbox("显示应用启动工具栏", 300, 248, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	If $bShowLaunchBar = 1 Then GUICtrlSetState($idShowLaunchBar, $GUI_CHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$idClearLicCaches = GUICtrlCreateCheckbox("修补后清除许可证缓存", 15, 308, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	...
	$idOlderVerDl = GUICtrlCreateCheckbox("旧版本下载", 300, 278, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	GUICtrlSetState($idOlderVerDl, $GUI_UNCHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	GUICtrlCreateGroup("", -99, -99, 1, 1)

	GUICtrlCreateGroup("Hosts 选项", 5, 342, 585, 90)

	$idCustomDomainListLabel = GUICtrlCreateLabel("Hosts 列表 URL：", 15, 367, 80, 20)
	...
	Global $idTriggerCaptureLaunch = GUICtrlCreateCheckbox("保存时触发一次性捕获并启动", 15, 397, 400, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	GUICtrlSetState($idTriggerCaptureLaunch, $GUI_UNCHECKED)

	GUICtrlCreateGroup("", -99, -99, 1, 1)

	GUICtrlCreateGroup("清理选项", 5, 435, 585, 60)
	...
	$idFinalCleanCheck = GUICtrlCreateCheckbox("最终 Adobe 清理检查", 300, 460, 275, 25, BitOR($BS_AUTOCHECKBOX, $BS_LEFT))
	GUICtrlSetState($idFinalCleanCheck, $GUI_UNCHECKED)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	GUICtrlCreateGroup("", -99, -99, 1, 1)

	$idOptionsReminder = GUICtrlCreateLabel("更改保存后才会生效", 10, 505, 575, 20, $SS_CENTER)
	...
	$idBtnSaveOptions = GUICtrlCreateButton("保存选项", 247, 540, 110, 32)
	GUICtrlSetImage(-1, "imageres.dll", 5358, 0)
	GUICtrlSetState($idBtnSaveOptions, $GUI_DISABLE)
	GUICtrlSetResizing(-1, $GUI_DOCKAUTO)

	$g_idHyperlinkOptions = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	Local $hWinTrustTab = GUICtrlCreateTabItem("信任")
	...
	$idBtnToggleWinTrust = GUICtrlCreateButton("切换 WinTrust", 227, 90, 140, 32)
	GUICtrlSetFont($idBtnToggleWinTrust, 9, 400, 0, "Segoe UI")

	$idBtnDevOverride = GUICtrlCreateButton("切换注册表键", 227, 135, 140, 32)
	GUICtrlSetFont($idBtnDevOverride, 9, 400, 0, "Segoe UI")

	$idBtnSetTrustPath = GUICtrlCreateButton("设置信任路径", 227, 225, 140, 32)
	...
	GUICtrlCreateLabel( _
			"通过信任使用 DLL 重定向的应用程序来减少弹窗。" & @CRLF & @CRLF & _
			"此功能自动管理所需的注册表项。" & @CRLF & @CRLF & _
			"您可以随时根据需要信任或取消信任应用程序。" & @CRLF & @CRLF & _
			"感谢 Team V.R。", _
	...
	$g_idHyperlinkWT = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$idBtnAutoUpdateHosts = GUICtrlCreateButton("定时更新", 227, 270, 140, 32)
	GUICtrlSetFont($idBtnAutoUpdateHosts, 9, 400, 0, "Segoe UI")

	GUICtrlCreateLabel( _
			"管理 hosts 文件以屏蔽与弹窗相关的域名。" & @CRLF & @CRLF & _
			"可从列表 URL 更新一次、手动编辑、或还原备份。" & @CRLF & @CRLF & _
			"使用定时更新可设置自动刷新的计划任务。" & @CRLF & @CRLF & _
			"保持 hosts 文件更新有助于持续维护保护。", _
			(595 - 580) / 2, 410, 580, 130, $SS_CENTER)
	GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

	$idBtnHostsInfo = GUICtrlCreateDummy()

	$g_idHyperlinkHosts = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$idBtnProxySetup = GUICtrlCreateButton("安装设置", 227, 90, 140, 32)
	GUICtrlSetFont($idBtnProxySetup, 9, 400, 0, "Segoe UI")

	$idBtnProxyTargeting = GUICtrlCreateButton("目标选择", 227, 135, 140, 32)
	GUICtrlSetFont($idBtnProxyTargeting, 9, 400, 0, "Segoe UI")
	GUICtrlSetState($idBtnProxyTargeting, $GUI_DISABLE)

	$idBtnProxyStartStop = GUICtrlCreateButton("启动/停止代理", 227, 180, 140, 32)
	...
	$g_idLblProxyStatus = GUICtrlCreateLabel("状态：（刷新中...）", 10, 330, 575, 20, $SS_CENTER)
	GUICtrlSetFont($g_idLblProxyStatus, 9, 700, 0, "Segoe UI")
	GUICtrlSetColor($g_idLblProxyStatus, 0x555555)

	GUICtrlCreateLabel( _
			"管理 mitmproxy 以拦截并中和 Adobe 心跳流量。" & @CRLF & @CRLF & _
			"安装设置会安装 mitmproxy 并信任证书。" & @CRLF & @CRLF & _
			"启动/停止代理用于运行代理。目标选择设置全局或指定应用。" & @CRLF & @CRLF & _
			"打开日志查看活动。卸载会完全卸载所有内容。" & @CRLF & @CRLF & _
			"感谢 MP7909。", _
			(595 - 580) / 2, 410, 580, 160, $SS_CENTER)
	GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

	$g_idHyperlinkProxy = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$idBtnToggleFW = GUICtrlCreateButton("切换规则", 227, 135, 140, 32)
	...
	$idBtnOpenWF = GUICtrlCreateButton("打开 Windows 防火墙", 227, 225, 140, 32)
	GUICtrlSetFont($idBtnOpenWF, 9, 400, 0, "Segoe UI")

	GUICtrlCreateLabel( _
			"管理防火墙规则以阻止应用程序访问互联网，可能减少弹窗。" & @CRLF & @CRLF & _
			"添加或删除出站规则、启用或禁用规则、或删除所有规则。" & @CRLF & @CRLF & _
			"注意：仅管理 Windows Defender 防火墙 - 第三方防火墙需手动设置。" & @CRLF & @CRLF & _
			"阻止互联网访问时，某些应用功能可能无法使用。", _
			(595 - 580) / 2, 410, 580, 130, $SS_CENTER)
	GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

	$idBtnFirewallInfo = GUICtrlCreateDummy()

	$g_idHyperlinkFW = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$hPopupTab = GUICtrlCreateTabItem("验证服务")
	...
	$idBtnRemoveAGS = GUICtrlCreateButton("移除 AGS", 227, 90, 140, 32)
	GUICtrlSetFont($idBtnRemoveAGS, 9, 400, 0, "Segoe UI")

	$idBtnDummyAGS = GUICtrlCreateButton("伪装 AGS", 227, 135, 140, 32)
	...
	GUICtrlCreateLabel( _
			"通过移除 AGS 组件来停止 'Adobe 正版服务提醒' 弹窗。" & @CRLF & @CRLF & _
			"或使用虚拟文件重定向服务以阻止后台活动。" & @CRLF & @CRLF & _
			"使用还原 AGS 可移除所有虚拟文件并还原为原始文件。" & @CRLF & @CRLF & _
			"注意：仅适用于标题为 'Genuine Service Alert' 的弹窗。", _
			(595 - 580) / 2, 410, 580, 130, $SS_CENTER)
	GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

	$idBtnAGSInfo = GUICtrlCreateDummy()

	$g_idHyperlinkAGS = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
	...
	$g_idHyperlinkLog = GUICtrlCreateLabel("GenP 百科与指南", (595 - 160) / 2, 580, 160, 24, BitOR($SS_CENTER, $SS_NOTIFY))
...
Func RecursiveFileSearch($INSTARTDIR, $DEPTH, $FileCount)
	...
	If 1 = Random(0, 10, 1) Then
		MemoWrite(@CRLF & "搜索 " & $FileCount & " 个文件" & @TAB & @TAB & "找到：" & UBound($g_aAllFiles) & @CRLF & _
				"---" & @CRLF & _
...
Func _PickExtraAdobeDrives($sWinDrv, $sMyDrv)
	...
	GUICtrlCreateLabel("扫描残留 Adobe .bak 文件的驱动器：", 10, 12, 260, 18)
	GUICtrlCreateLabel("（已覆盖的驱动器将被排除）", 10, 30, 260, 16)
...
Func _RunFinalCleanCheck()
	...
	If CheckThirdPartyFirewall() Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  防火墙", _
				"此系统上有第三方防火墙处于活动状态。" & @CRLF & @CRLF & _
				"GenP 无法自动检查或移除 Adobe 防火墙规则。" & @CRLF & @CRLF & _
				"请打开您的防火墙软件，手动移除所有 Adobe 出站规则，" & @CRLF & _
				"然后点击确定继续。")
	ElseIf _WinFirewallServiceReady() Then
		...
		If $iFWRemoved > 0 Then
			...
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  防火墙", _
					"防火墙规则：已移除 " & $iFWRemoved & " 条 GenP Adobe 规则。" & @CRLF & @CRLF & _
					"点击确定继续。")
		Else
			$sReport &= "Firewall rules:   None found." & @CRLF
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  防火墙", _
					"防火墙规则：未找到  -  全部清除。" & @CRLF & @CRLF & _
					"点击确定继续。")
		EndIf
	Else
	...
	GUICtrlSetData($idStepLabel, "检查代理和 mitmproxy...")
	...
	ElseIf _IsWindowsProxyOn() Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  代理", _
				"代理：Windows 系统代理已禁用。" & @CRLF & @CRLF & _
				"点击确定继续。")
	Else
		$sReport &= "Proxy/mitmproxy:  Not configured." & @CRLF
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  代理", _
				"代理 / mitmproxy：未配置  -  全部清除。" & @CRLF & @CRLF & _
				"点击确定继续。")
	EndIf

	GUICtrlSetData($idStepLabel, "移除计划任务和日志文件...")
	...
	MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  计划任务", _
			"计划任务：" & @CRLF & @CRLF & _
			($sTaskReport <> "" ? $sTaskReport & "。" : "未找到 GenP 任务  -  全部清除。") & @CRLF & @CRLF & _
			"点击确定继续。")

	GUICtrlSetData($idStepLabel, "清理 hosts 文件...")
	...
	If $bHasAdobeLines Then
		...
		If FileExists($sHostsPlain) Then
			...
			MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  Hosts 文件", _
					"在您的 hosts 文件中仍发现 Adobe 条目。" & @CRLF & @CRLF & _
					"您的原始备份 (hosts.plain) 存在。请将您的" & @CRLF & _
					"hosts 文件替换为以下之一：" & @CRLF & @CRLF & _
					"  1) 您的备份文件位于：" & @CRLF & _
					"     C:\Windows\System32\drivers\etc\hosts.plain" & @CRLF & @CRLF & _
					"  2) 仅包含以下内容的干净默认文件：" & @CRLF & _
		...
		Else
			...
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  Hosts 文件", _
					"在您的 hosts 文件中发现了 Adobe 条目。" & @CRLF & @CRLF & _
					"您的 hosts 文件已被替换为干净的默认文件。" & @CRLF & @CRLF & _
					"点击确定确认并继续。")
		EndIf
	Else
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  Hosts 文件", _
				"Hosts 文件是干净的。" & @CRLF & @CRLF & _
				$sHostsMsg & @CRLF & @CRLF & _
				"点击确定继续。")
		If FileExists($sEtcDir & "hosts.plain") Then
			MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  Hosts 文件", _
					"从 GenP 的更新 Hosts 功能中发现了一个 hosts.plain 备份。" & @CRLF & @CRLF & _
					"您的 hosts 文件看起来是干净的，但此备份仍存在。请选择：" & @CRLF & @CRLF & _
					"  1) 从中还原以恢复原始 hosts 文件" & @CRLF & _
					"  2) 如果当前 hosts 文件已正确则删除它" & @CRLF & @CRLF & _
	...
	If $sEtcBaks <> "" Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  Hosts 备份", _
				"在您的 hosts 目录中发现残留备份文件。" & @CRLF & @CRLF & _
				"请手动删除这些文件，然后点击确定继续：" & @CRLF & @CRLF & _
				$sEtcBaks)
	EndIf
	FileSetAttrib($sHostsPath, "+R")
	$sReport &= "Hosts file:       " & $sHostsMsg & @CRLF

	GUICtrlSetData($idStepLabel, "移除残留 .bak 文件...")
	...
	MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  备份文件", _
			".bak 备份文件：" & ($iBakTotal > 0 ? $iBakTotal & " 个已移除。" : "未找到  -  全部清除。") & @CRLF & @CRLF & _
			"点击确定继续。")

	GUICtrlSetData($idStepLabel, "检查剩余 Adobe 文件夹...")
	...
	If $sRemaining <> "" Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  Adobe 文件夹", _
				"以下 Adobe 安装文件夹仍然存在。" & @CRLF & _
				"请通过 Creative Cloud 或控制面板卸载，或手动删除。" & @CRLF & _
				"清除后点击确定继续：" & @CRLF & @CRLF & _
				$sRemaining)
	Else
		$sReport &= "Adobe folders:    None found in set path." & @CRLF
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  Adobe 文件夹", _
				"Adobe 安装文件夹：全部清除。" & @CRLF & @CRLF & _
				"点击确定继续。")
	EndIf

	GUICtrlSetData($idStepLabel, "检查 AppData 和 Temp 中的 Adobe 文件夹...")
	...
	If $sUserFolders <> "" Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  AppData 文件夹", _
				"在用户目录中发现以下 Adobe 文件夹。" & @CRLF & _
				"请手动删除这些文件，然后点击确定继续：" & @CRLF & @CRLF & _
				$sUserFolders)
	Else
		$sReport &= "AppData folders:  No Adobe folders found." & @CRLF
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  AppData 文件夹", _
				"AppData 和 Temp 中的 Adobe 文件夹：全部清除。" & @CRLF & @CRLF & _
				"点击确定继续。")
	EndIf

	GUICtrlSetData($idStepLabel, "还原 WinTrust 注册表键...")
	...
	If $bWTNeedsRestore Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  WinTrust", _
				"WinTrust 注册表键：" & @CRLF & @CRLF & _
				$sWTMsg & @CRLF & @CRLF & _
				"点击确定确认并继续。")
	Else
		$sReport &= "WinTrust keys:    Already at secure defaults." & @CRLF
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  WinTrust", _
				"WinTrust 注册表键：已处于安全默认值。" & @CRLF & @CRLF & _
				"点击确定继续。")
	EndIf

	GUICtrlSetData($idStepLabel, "检查注册表中的 Adobe 条目...")
	...
	If $sRegKeys <> "" Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONEXCLAMATION), "最终 Adobe 清理检查  -  注册表", _
				"在注册表中发现 Adobe 条目。" & @CRLF & _
				"请打开注册表编辑器 (regedit)，移除以下键，" & @CRLF & _
				"然后点击确定继续：" & @CRLF & @CRLF & _
				$sRegKeys)
	Else
		$sReport &= "Registry:         No Adobe entries found." & @CRLF
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查  -  注册表", _
				"注册表：未发现 Adobe 条目  -  全部清除。" & @CRLF & @CRLF & _
				"点击确定继续。")
	EndIf

	GUICtrlSetData($idStepLabel, "还原 CCX Start 主页为 Adobe 默认...")
	_RestoreCcxStart()
	$sReport &= "CCX Start home:   restored to Adobe defaults (pinned copy removed, .orig cleared)." & @CRLF

	GUICtrlSetData($idStepLabel, "清除 Adobe 许可证缓存（完全重置）...")
	...
	MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "最终 Adobe 清理检查", $sReport)
...
Func _PrecacheDisplayAssets()
	...
	GUICtrlCreateLabel("欢迎使用 GenP v4.2.1", 0, 25, $iGuiWidth, 20, 1)
	GUICtrlSetFont(-1, 10, 700, 0, "Segoe UI")

	GUICtrlCreateLabel("原作者：uncia", 0, 48, $iGuiWidth, 20, 1)
	...
	Local $hHyperlink = GUICtrlCreateLabel("GenP 百科与指南", 0, 140, $iGuiWidth, 20, 0x0101)
...
Func FillListViewWithInfo()
	...
	$line += 1
	_GUICtrlListView_SetItemText($g_idListview, $line, "原作者：uncia", 1)
	$line += 1
	...
	_GUICtrlListView_SetItemText($g_idListview, $line, "--------------------", 1)
	$line += 2
	_GUICtrlListView_SetItemText($g_idListview, $line, "当前搜索路径：", 1)
	$line += 1
	_GUICtrlListView_SetItemText($g_idListview, $line, $MyDefPath, 1)
	$line += 1
	_GUICtrlListView_SetItemText($g_idListview, $line, "按 '路径' 更改搜索位置", 1)

	$line += 2
	_GUICtrlListView_SetItemText($g_idListview, $line, "按 '搜索' 扫描已安装的应用程序", 1)
	$line += 1
	_GUICtrlListView_SetItemText($g_idListview, $line, "按 '修补' 对选中的文件应用补丁", 1)
...
Func FillListViewWithFiles()
	...
	If UBound($g_aAllFiles) > 0 Then
		MemoWrite(@CRLF & UBound($g_aAllFiles) & " 个文件在 " & Round(TimerDiff($timestamp) / 1000, 0) & " 秒内找到于：" & @CRLF & "---" & @CRLF & $MyDefPath & @CRLF & "---" & @CRLF & "按 '修补' 应用补丁")
	...
	Else
		MemoWrite(@CRLF & "未找到任何内容" & @CRLF & "---" & @CRLF & $MyDefPath & @CRLF & "---" & @CRLF & "等待用户操作")
...
Func _ShowEmptyModifiedNotice()
	...
	_GUICtrlListView_SetItemText($g_idListview, 2, "所有文件已修补。", 1)
	_GUICtrlListView_SetItemText($g_idListview, 3, "--------------------", 1)
	_GUICtrlListView_SetItemText($g_idListview, 4, "修改工作队列已清空。", 1)
	_GUICtrlListView_SetItemText($g_idListview, 6, "3 秒后返回主界面...", 1)
...
Func _ApplyModifiedFilter()
	...
	MemoWrite(@CRLF & "修改过滤：" & $iKept & " 个文件需要修补（隐藏了 " & $iRemoved & " 个已修补）。")
...
Func _LockOptionsUIForScan()
	If $idOptionsReminder > 0 Then
		GUICtrlSetData($idOptionsReminder, "正在扫描您设置的路径，请稍候...")
...
Func _UnlockOptionsUIAfterScan()
	If $idOptionsReminder > 0 Then
		GUICtrlSetData($idOptionsReminder, "更改保存后才会生效")
		GUICtrlSetState($idOptionsReminder, $GUI_HIDE)
	EndIf
	If $g_idOptionsProgress > 0 Then
...
Func _VerifyListedFiles($bSilent = False)
	...
	If $iCount = 0 Then
		If Not $bSilent Then MemoWrite(@CRLF & "没有需要验证的内容。")
		Return
	EndIf
...
Func _ShowStatusScreen($sMode, $sDir = "")
	...
	Switch $sMode
		Case "scanning"
			_GUICtrlListView_SetItemText($g_idListview, 1, "正在扫描已安装的应用程序：", 1)
			_GUICtrlListView_SetItemText($g_idListview, 2, "正在检查：" & $sDisplayDir, 1)
			_GUICtrlListView_SetItemText($g_idListview, 3, "--------------------", 1)
			_GUICtrlListView_SetItemText($g_idListview, 4, "找到应用：" & $g_AppCount, 1)
			_GUICtrlListView_SetItemText($g_idListview, 5, "符合条件的文件：" & _FormatNumber($g_FilesToPatchCount), 1)
		...
		Case "complete"
			...
			_GUICtrlListView_SetItemText($g_idListview, 5, "符合条件的文件：" & _FormatNumber($g_FilesToPatchCount), 1)
			_GUICtrlListView_SetItemText($g_idListview, 6, "", 1)
			_GUICtrlListView_SetItemText($g_idListview, 7, "正在加载检测到的应用...", 1)
			_GUICtrlListView_SetItemText($g_idListview, 8, "", 1)

		Case "stopped"
			_GUICtrlListView_SetItemText($g_idListview, 1, "正在扫描已安装的应用程序：", 1)
			_GUICtrlListView_SetItemText($g_idListview, 2, "扫描已被用户停止。", 1)
			_GUICtrlListView_SetItemText($g_idListview, 3, "--------------------", 1)
			_GUICtrlListView_SetItemText($g_idListview, 4, "找到应用：" & $g_AppCount, 1)
			_GUICtrlListView_SetItemText($g_idListview, 5, "符合条件的文件：" & _FormatNumber($g_FilesToPatchCount), 1)
			_GUICtrlListView_SetItemText($g_idListview, 6, "", 1)
		...
		If StringInStr($sData, "|") Then
			...
			If UBound($aArray) = 3 Then
				...
				If $iPatternLength <> StringLen($sReplace) Or Mod($iPatternLength, 2) <> 0 Then
					MsgBox($MB_SYSTEMMODAL, "错误", "config.ini 中的模式错误：" & $sPattern & @CRLF & $sSearch & @CRLF & $sReplace)
					Exit
				EndIf
...
Func _PatchLargeFileWithPatterns($sFilePath)
	...
	For $i = 0 To UBound($aPatNames) - 1
		...
		If StringLen($sSearch) <> StringLen($sReplace) Or Mod(StringLen($sSearch), 2) <> 0 Then
			MsgBox($MB_SYSTEMMODAL, "错误", "大文件模式长度不匹配：" & $sName & @CRLF & $sSearch & @CRLF & $sReplace)
			Return -1
		EndIf
...
Func _ReconcilePatchStates()
	...
	If Not FileExists($patchStatesINI) Then
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "核对", "没有 patch_states.ini - 无需核对。")
		Return $aResult
	EndIf
	...
	If $hStateProbe = -1 Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONERROR), "无法核对", _
				"patch_states.ini 无法写入：" & @CRLF & @CRLF & $patchStatesINI & @CRLF & @CRLF & _
				"它可能是只读的或在其他程序中打开。请修复后重试。" & @CRLF & _
				"未进行任何更改。")
		Return $aResult
	EndIf
	...
	ToggleLog(0)
	MemoWrite(@CRLF & "正在核对 patch_states.ini 与当前安装...")
	...
	If $iTotal = 0 Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "核对", "在 " & $MyDefPath & " 中未找到符合条件的文件。" & @CRLF & "核对已中止。")
	...
	For $i = 0 To $iTotal - 1
		...
		If Mod($i, 10) = 0 Then
			...
			_ShowStatusScreen("patching", "正在核对：" & StringRegExpReplace($sPath, "^.*\\", ""))
		EndIf
	...
	Local $sSummary = "核对完成。" & @CRLF & @CRLF & _
			"磁盘扫描文件数：    " & $iTotal & @CRLF & _
			"INI 条目数：           " & $aResult[3] & @CRLF & _
			"确认准确：       " & $aResult[2] & @CRLF & _
			"偏差已纠正：          " & $aResult[1] & @CRLF & _
			"缺失（已移除）：        " & $aResult[0] & @CRLF & _
			"新增文件：          " & $iNew & @CRLF & @CRLF & _
			"请查看主界面标签页确认结果符合您的" & @CRLF & _
			"预期。如果有异常，可使用修补或还原" & @CRLF & _
	...
	MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "核对完成", $sSummary)
...
Func _PromptStopAdobeProcessesForOp($sOpName)
	...
	Local $iAns = MsgBox(BitOR($MB_YESNOCANCEL, $MB_ICONWARNING, $MB_SYSTEMMODAL), _
			"检测到 Adobe 进程", _
			$iAdobeCount & " 个 Adobe 相关进程正在运行。" & @CRLF & @CRLF & _
			"打开的 Adobe 应用、正在运行的进程或服务可能锁定" & @CRLF & _
			"文件并阻止其被" & $sOpName & "。" & @CRLF & @CRLF & _
			"是：GenP 将自动停止所有正在运行的 Adobe 进程" & @CRLF & _
			"          和服务，然后继续。" & @CRLF & _
			"否：仍然继续（正在使用的文件将被自动" & @CRLF & _
			"          跳过并报告给您）。" & @CRLF & _
			"取消：中止，不会更改任何内容。")
	If $iAns = $IDCANCEL Then
	...
	If $iAns = $IDYES Then
		...
		Else
			...
			MsgBox(BitOR($MB_OK, $MB_ICONWARNING), "部分进程未被终止", _
					$iSurvived & " 个 Adobe 进程无法被终止。" & @CRLF & @CRLF & _
					"继续 - 锁定的文件将被跳过并报告。")
		EndIf
	Else
...
Func _WipeAdobeLicenseCaches()
	...
	LogWrite(1, "清除 Adobe 许可证缓存（完全重置）...")
	MemoWrite(@CRLF & "正在清除 Adobe 许可证缓存：")
	...
	MemoWrite("已清除：Windows 凭据库 Adobe 令牌。")

	If $iFailed > 0 Then
		LogWrite(1, "Cache wipe finished: " & $iWiped & " removed, " & $iFailed & " still locked.")
		Local $iLocked = MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING, $MB_DEFBUTTON2), "部分缓存仍被锁定", _
				$iWiped & " 项已移除，但 " & $iFailed & " 项仍在使用中" & @CRLF & _
				"且无法清除（见日志）。" & @CRLF & @CRLF & _
				"要完全重置，请关闭所有 Adobe 应用和" & @CRLF & _
				"Creative Cloud，然后再次运行此过程。" & @CRLF & @CRLF & _
				"现在停止以便关闭它们？" & @CRLF & @CRLF & _
				"是 = 停止（尚未修补）。" & @CRLF & _
				"否 = 仍然继续（锁定项保持原样）。")
...
Func _CreateInitialPatchStates()
	If FileExists($patchStatesINI) Then
		MsgBox(BitOR($MB_OK, $MB_ICONERROR), "无法创建", "patch_states.ini 已存在。请使用核对功能。")
		Return
	EndIf
	...
	If $iSurvived > 0 Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "进程仍在运行", _
				$iSurvived & " 个 Adobe 进程无法被终止：" & @CRLF & @CRLF & _
	...
	_LockOptionsUIForScan()
	ToggleLog(0)
	MemoWrite(@CRLF & "正在构建初始 patch_states.ini - 扫描文件...")
	...
	If UBound($g_aAllFiles, 1) = 0 Then
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "创建新文件", "在 " & $MyDefPath & "。" & @CRLF & "patch_states.ini 未创建。")
...
Func _BuildAppsToolbar()
	...
	If $iCount = 0 Then
		...
		GUICtrlCreateLabel("应用启动工具栏", $iPad, $iPad, $iW - $iPad * 2, 16)
		...
		Else
			$sHint = "未找到已配置的应用。点击" & @CRLF & "配置来更新您的选择。"
		EndIf
	...
	$g_hAppsBar = GUICreate("GenP v" & $g_Version & " Toolbar", $iW, $iH, $iX, $iY, _
			BitOR($WS_CAPTION, $WS_POPUP, $WS_SYSMENU), $WS_EX_TOPMOST, $MyhGUI)
	If FileExists(@ScriptDir & "\Skull.ico") Then GUISetIcon(@ScriptDir & "\Skull.ico", 0, $g_hAppsBar)
	GUICtrlCreateLabel("打开应用（绕过 CC 检查）：", $iPad, $iPad, $iW - $iPad * 2, 16)
	GUICtrlSetFont(-1, 7, 600, 0, "Segoe UI")
...
Func _ShowToolbarConfigDialog()
	...
	If $iAllCount = 0 Then
		MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION, $MB_SYSTEMMODAL), "配置工具栏", _
				"在 patch_states.ini 中未找到已修补的应用。" & @CRLF & @CRLF & _
				"请先修补一些应用，然后配置工具栏。")
		Return
	EndIf
	...
	GUICtrlCreateLabel("选择最多 10 个要显示在工具栏上的应用：", 10, 10, $iDW - 20, 30)
	GUICtrlSetFont(-1, 7, 600, 0, "Segoe UI")
		...
		If $iDlgMsg = $idOK Then
			...
			If $iChecked = 0 Then
				MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "配置工具栏", _
						"请至少选择一个应用，或点击取消以不做更改关闭。")
				ContinueLoop
			EndIf

			If $iChecked > 10 Then
				MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "配置工具栏", _
						"您已选择 " & $iChecked & " 个应用。最多为 10 个。" & @CRLF & @CRLF & _
						"请取消选择 " & ($iChecked - 10) & " 个应用后重试。")
				ContinueLoop
			EndIf
...
Func hL_WM_COMMAND($hWnd, $iMsg, $wParam, $lParam)
	...
	If $g_bMitmLogWindowExists And $hWnd = $g_hMitmLogWin Then
		...
		ElseIf $iIDFrom = $g_idBtnLogClear Then
			...
			_AppendToLogWindow("# 日志已清除。" & @CRLF & @CRLF)
			Return $GUI_RUNDEFMSG
		...
		ElseIf $iIDFrom = $g_idBtnLogAddToHosts Then
			...
			If @error Then
				MsgBox(48, "警告", "在当前日志数据中未找到活动的 Adobe 遥测域名。")
				Return $GUI_RUNDEFMSG
			EndIf
			...
			If _SilentHostsInjector($sTargetDomain) Then
				MsgBox(64, "成功", "Adobe 端点屏蔽更新成功（手动）！" & @CRLF & @CRLF & _
						"屏蔽规则条目：" & @CRLF & _
						"0.0.0.0 ic.adobe.io" & @CRLF & _
						"0.0.0.0 " & $sTargetDomain & @CRLF & @CRLF & _
						"规则已保存到您的 hosts 文件并已刷新本地 DNS 缓存。")
			Else
				MsgBox(16, "错误", "无法写入 hosts 文件。" & @CRLF & @CRLF & "请确保应用以管理员身份运行。")
			EndIf
...
Func SaveOptionsToConfig()
	If _IsChecked($idResetOnSave) Then
		Local $iConfirm = MsgBox(BitOR($MB_YESNO, $MB_ICONEXCLAMATION), _
				"重置修补状态？", _
				"删除 patch_states.ini？所有记录的修补/还原历史" & @CRLF & _
				"（MD5 哈希、修补/未修补标志、应用版本）将丢失。" & @CRLF & @CRLF & _
				"您磁盘上实际已修补的文件不受影响 - 仅影响 GenP 的" & @CRLF & _
				"内部修补记录。")
	...
	If _IsChecked($idCreateStates) Then
		If FileExists($patchStatesINI) Then
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "无法创建新 patch_states.ini", _
					"patch_states.ini 已存在于：" & @CRLF & $patchStatesINI & @CRLF & @CRLF & _
					"请使用 '核对导入的 patch_states.ini'，它会验证" & @CRLF & _
					"现有文件与您当前安装的一致性。")
		Else
	...
	If $bShowLaunchBar Then
		If Not FileExists($patchStatesINI) Then
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION, $MB_SYSTEMMODAL), "启动工具栏", _
					"未找到 patch_states.ini。" & @CRLF & @CRLF & _
					"在创建之前不会显示工具栏。" & @CRLF & _
					"请在首次运行选项中使用 '创建并填充新的 patch_states.ini'。")
			$bShowLaunchBar = 0
		...
		ElseIf Not _PatchStatesSetupDone() Then
			MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "启动工具栏", _
					"patch_states.ini 存在但设置未验证。" & @CRLF & @CRLF & _
					"工具栏需要完成以下操作之一：" & @CRLF & @CRLF & _
					"  - 创建新 patch_states.ini，或" & @CRLF & _
					"  - 核对导入的 patch_states.ini" & @CRLF & @CRLF & _
					"请在首次运行选项中执行相应操作并再次保存。")
			$bShowLaunchBar = 0
			IniWrite($sINIPath, "Options", "ShowLaunchBar", "0")
			GUICtrlSetState($idShowLaunchBar, $GUI_UNCHECKED)
		EndIf
	...
	If _IsChecked($idUseCustomDefault) Then
		...
		Else
			MsgBox($MB_OK, "未设置自定义路径", _
					"已勾选 '使用自定义默认搜索路径' 但路径为空或无效。" & @CRLF & @CRLF & _
					"下次启动时工具将恢复为 " & $sDefaultPath & "。" & @CRLF & _
					"请设置有效文件夹并再次保存以启用自定义路径。")
	...
	If $MyDefPath <> $sOldDefPath Then
		...
		MemoWrite(@CRLF & "搜索路径已更新为：" & @CRLF & "---" & @CRLF & $MyDefPath & @CRLF & "---" & @CRLF & "点击搜索按钮扫描新位置。")
	...
	If $sNewDomainListURL = "" Then
		...
		MsgBox(0, "URL 为空", "自定义域名列表 URL 不能为空。已设置默认 URL。")
	EndIf
...
Func ScanDNSCache(ByRef $sHostsContent)
	...
	Local $sPrompt = "在 DNS 缓存中发现 " & UBound($aNewDomains) & " 个新域名：" & @CRLF & _
			_ArrayToString($aNewDomains, @CRLF) & @CRLF & "添加到 hosts 文件？"
	...
	If $iMethod = $g_iHAU_METHOD_BASIC Then
		If StringInStr($sGenPExePath, "\Users\") Or _
			...
			Local $iAns = MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING), _
					"Hosts 自动更新设置", _
					"GenP 似乎位于用户特定文件夹中：" & @CRLF & _
					$sGenPExePath & @CRLF & @CRLF & _
					"计划任务以 SYSTEM 身份运行，通常无法访问" & @CRLF & _
					"用户文件夹。任务将被创建但可能无法执行。" & @CRLF & _
					"尝试运行时。" & @CRLF & @CRLF & _
					"建议：先将 GenP 移动到根目录如 C:\GenP\。" & @CRLF & @CRLF & _
					"仍然继续？")
...
Func _ShowGudeSetupPrompt()
	Local $iAns = MsgBox(BitOR($MB_YESNOCANCEL, $MB_ICONQUESTION, $MB_SYSTEMMODAL), _
			"Gude 日志清理", _
			"对于某些用户，Good 补丁可能导致 Adobe 在" & @CRLF & _
			"每次打开修补后的应用时，在 Adobe 文件夹中生成" & @CRLF & _
			"gude-YYYY-MM-DD.log 文件。这些是无害的副作用，" & @CRLF & _
			"可以自动删除。" & @CRLF & @CRLF & _
			"GenP 可以添加一个计划任务每 3 小时删除一次，" & @CRLF & _
			"并在每次 Windows 登录时执行。" & @CRLF & @CRLF & _
			"是 - 选择驱动器并设置任务" & @CRLF & _
			"否 - 跳过（不再询问）" & @CRLF & _
			"取消 - 下次再提醒")
	Select
...
Func _ShowGudeRemovePrompt()
	Local $iAns = MsgBox(BitOR($MB_YESNOCANCEL, $MB_ICONQUESTION, $MB_SYSTEMMODAL), _
			...
			"由于您已禁用 Good 补丁，是否" & @CRLF & _
			"同时移除 Gude 日志清理任务？" & @CRLF & @CRLF & _
			"是 - 移除计划任务" & @CRLF & _
			"否 - 保留运行（不再询问）" & @CRLF & _
			"取消 - 下次再提醒")
	Select
...
Func _ShowHostsAutoUpdateDialog()
	...
	Local $hDlg = GUICreate("Hosts 定时更新", $iW, $iH, -1, -1, _
	...
	Local $idLblStatus = GUICtrlCreateLabel("（检查中...）", 70, 15, 430, 18)
	GUICtrlSetFont($idLblStatus, 9, 700)

	GUICtrlCreateGroup("方式", 15, 45, 490, 175)
	Local $idRadioBasic = GUICtrlCreateRadio("基础 - 每日刷新，简单", 30, 70, 460, 22)
	GUICtrlCreateLabel( _
			"每天运行一次 GenP.exe -updatehosts。无备份，无恢复。" & @CRLF & _
			"效果与在计划时间点击更新 Hosts 按钮相同。", _
			50, 92, 450, 32)
	Local $idRadioAdvanced = GUICtrlCreateRadio("高级 - 每 3 小时，含备份、恢复和自定义条目合并", 30, 130, 460, 22)
	GUICtrlCreateLabel( _
			"通过 PowerShell 脚本每 3 小时运行。每次更新前备份 hosts，" & @CRLF & _
			"验证新列表，失败时恢复，记录所有操作。" & @CRLF & _
			"合并 hosts.plain 中的条目（如有）以保留自定义条目。", _
			50, 152, 450, 60)
	GUICtrlCreateGroup("", -99, -99, 1, 1)
	...
	Local $idBtnRemove = GUICtrlCreateButton("移除", 255, 290, 110, 32)
	...
	While Not $bExit
		...
		Switch $iMsg
				...
				If _RunHostsAutoUpdateNow() Then
					...
					If $bChanged Then
						...
						MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "立即运行", _
								"任务已完成。" & @CRLF & @CRLF & _
								"最后一条日志：" & @CRLF & $sLastLine)
					Else
						MsgBox(BitOR($MB_OK, $MB_ICONWARNING), "立即运行", _
								"任务已触发但 8 秒内未检测到日志更新。" & @CRLF & _
								"可能仍在运行 - 请稍后检查日志文件：" & @CRLF & _
								$g_sHAU_LOG_TARGET)
					EndIf
				Else
			...
			Case $idBtnRemove
				If MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING), "移除", _
						"移除计划任务？" & @CRLF & _
						"（高级方式：同时移除 UpdateHostsFile.ps1 及其日志。）") = $IDYES Then
					If _RemoveHostsAutoUpdate(True) Then
						MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION), "已移除", "计划任务已移除。")
					Else
						MsgBox(BitOR($MB_OK, $MB_ICONERROR), "移除", "移除失败 (@error=" & @error & ").")
					EndIf
					_RefreshAutoUpdateDialogStatus($idLblStatus, $idBtnRunNow, $idBtnRemove)
				EndIf
		EndSwitch
...
Func _ShowProxyTargetingDialog()
	...
	Local $hDlg = GUICreate("代理目标选择", $iDW, $iDH, -1, -1, _
		BitOR($WS_CAPTION, $WS_POPUP, $WS_SYSMENU), $WS_EX_TOPMOST, $MyhGUI)
	If FileExists(@ScriptDir & "\Skull.ico") Then GUISetIcon(@ScriptDir & "\Skull.ico", 0, $hDlg)

	Local $idRadGen = GUICtrlCreateRadio("全局 - 所有 Adobe 流量（推荐）", 12, 10, $iDW - 24, 18)
	Local $idRadTgt = GUICtrlCreateRadio("指定 - 仅勾选下方的应用（最多 4 个）", 12, 30, $iDW - 24, 18)
	If $bTargeted Then
	...
	Local $idSave   = GUICtrlCreateButton("保存",   12,        $iBtnY, $iBW, 26)
	Local $idCancel = GUICtrlCreateButton("取消", 24 + $iBW, $iBtnY, $iBW, 26)
	...
	While True
		...
		If $iM = $idSave Then
			...
			GUIDelete($hDlg)
			Local $sBody = "已保存。" & @CRLF & @CRLF & _
				"Mode: " & ($sMode = "Target" ? "Targeted (" & $sApps & ")" : "Global (recommended)") & "."
			If $iN > 4 Then $sBody &= @CRLF & @CRLF & "（您勾选了 " & $iN & " 个应用；最多为 4 个，因此使用全局模式。）"
			$sBody &= @CRLF & @CRLF & "如果代理正在运行，请使用启动/停止代理来重启并应用更改。"
			MsgBox(BitOR($MB_OK, $MB_ICONINFORMATION, $MB_SYSTEMMODAL), "代理目标选择", $sBody)
			Return
		EndIf
	WEnd
	GUIDelete($hDlg)
...
Func _CreateMitmproxyLogWindow()
	...
	$g_idLblLogStatus = GUICtrlCreateLabel("（未启动）", 65, 14, 250, 18)
	...
	$g_idBtnLogClear = GUICtrlCreateButton("清除", $iW - 305, 8, 90, 28)
	...
	$g_idBtnLogAddToHosts = GUICtrlCreateButton("添加 hosts", $iW - 205, 8, 90, 28)
	...
	_AppendToLogWindow("# GenP 心跳抑制 - 实时日志" & @CRLF)
	_AppendToLogWindow("# 此窗口实时流式显示 mitmdump 的标准输出/标准错误。" & @CRLF)
	_AppendToLogWindow("# 启动代理后开始输出。" & @CRLF & @CRLF)
EndFunc
...
Func _TriggerOneOffCaptureAndLaunch()
	...
	If $bCaptured Then
		MsgBox(64, "成功", "通过 " & StringReplace($sAppName, ".exe", "") & " 更新 Adobe 端点屏蔽成功！" & @CRLF & @CRLF & _
				"屏蔽规则条目：" & @CRLF & _
				"0.0.0.0 ic.adobe.io" & @CRLF & _
	...
	Else
		...
		MsgBox(48, "捕获失败", "从 " & $sAppName & " 获取后台令牌超时。" & @CRLF & _
				"请正常运行您的 Adobe 应用，然后从日志窗口手动点击" & @CRLF & "'添加 hosts'。")
	EndIf
	Return $bCaptured
EndFunc
...
Func RestoreHosts()
	...
	_GUICtrlTab_SetCurFocus($hTab, $g_iLogTabIndex)
	MemoWrite(@CRLF & "正在从备份还原 hosts 文件..." & @CRLF & "---" & @CRLF & "请稍候..." & @CRLF)
	Local $sHostsPath = @WindowsDir & "\System32\drivers\etc\hosts"
	Local $sBackupPath = @WindowsDir & "\System32\drivers\etc\hosts.bak"
...
Func _EnableNGLFirewallRules($bFocusLogOnDone = True)
	...
	If CheckThirdPartyFirewall() Then
		...
		Local $iGo = MsgBox(BitOR($MB_YESNO, $MB_ICONWARNING, $MB_SYSTEMMODAL), "检测到第三方防火墙", _
				"第三方防火墙 (" & $g_sThirdPartyFirewall & ") 处于活动状态，" & @CRLF & _
				"GenP 无法自动添加 NGL 屏蔽规则。" & @CRLF & @CRLF & _
				"需要屏蔽的文件列表已在记事本中打开。" & @CRLF & _
				"请在您自己的防火墙中为每个文件添加" & @CRLF & _
				"出站屏蔽规则。" & @CRLF & @CRLF & _
				"现在继续修补？" & @CRLF & @CRLF & _
				"是 = 继续（添加规则由您自行完成）。" & @CRLF & _
				"否 = 取消，以便添加规则后再次修补。")
		If $iGo = $IDNO Then
	...
	If Not _WinFirewallServiceReady() Then
		...
		MsgBox(BitOR($MB_OK, $MB_ICONWARNING, $MB_SYSTEMMODAL), "Windows 防火墙不可用", _
				"Windows 防火墙服务已关闭或不可用，" & @CRLF & "因此 GenP 无法添加 NGL 屏蔽规则。" & @CRLF & @CRLF & _
				"您的应用已修补且可以使用，但 NGL 未进行网络隔离。" & @CRLF & _
				"如果需要隔离，请重新打开 Windows 防火墙并再次修补。这由您自行决定。")
		If $bFocusLogOnDone Then ToggleLog(1)
		Return 0
	EndIf
...
Func UnpackRuntimeInstallers()
	...
	For $file In $selectedFiles
		...
		If Not IsUPXPacked($file) Then
			MemoWrite("跳过：" & $file & " 不是 UPX 打包的文件。")
			LogWrite(1, "跳过：" & $file & " 不是 UPX 打包的文件。")
			ContinueLoop
		EndIf
...
Func RemoveDevOverride()
	...
	MemoWrite("已禁用 WinTrust 覆盖；所有四个键已恢复默认安全设置。")
	...
	For $app In $SelectedApps
		...
		If StringRegExp($app, "(?i)\\AfterFX( \(Beta\))?\.exe$") Then
			...
			If _IsAEBlockedByVersion($aAEVer[0], $aAEVer[1]) Then
				MemoWrite("跳过 " & $app & " - 无 WinTrust (After Effects v" & $aAEVer[0] & "." & $aAEVer[1] & "+ 已被阻止)。")
				LogWrite(1, "跳过 AE 阻止的路径：" & $app & " (v" & $aAEVer[0] & "." & $aAEVer[1] & ")")
				ContinueLoop
			EndIf
		EndIf
...
Func ManageDevOverride()
	...
	If ($iIFEOErr = 0 And $iIFEO = 1) And ($iSxSErr = 0 And $iSxS = 1) _
		...
		$sOverallState = "四个键均已设置 - 覆盖完全激活。"
	ElseIf $iIFEOErr <> 0 And $iSxSErr <> 0 And $iWT64Err <> 0 And $iWT32Err <> 0 Then
		$sOverallState = "未设置任何键 - 覆盖已禁用。"
	Else
		$sOverallState = "部分状态 - 某些键已设置，其他未设置。"
	EndIf
